import numpy as np
from Adaboost import createLines
...
def fit(points, labels, num_iterations=50):
    ...
    POINT_WEIGHTS = [1 / len(points)] * len(points)
    ...
    return rules, alphas, total_train_errors, total_test_errors
...
def cumulative_Hx(lines, alphas, p3):
    ...
    label = 1 if sum([alphas[i] * hx(lines[i], p3) for i in range(len(lines))]) > 0 else -1
    return label
...
def write_to_file(file_name, rules, alphas):
    ...
    data_folder = Path("C:/Users/netan/PycharmProjects/ML_Ex/ex3/data/")
    file_to_open = data_folder / f'{file_name}_output'
